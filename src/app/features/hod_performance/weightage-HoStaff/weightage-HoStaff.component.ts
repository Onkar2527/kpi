...
export class WeightageHoStaffComponent implements OnInit {
    ...
    this.periodService.currentPeriod.subscribe(period => {
      ...
      if (this.HODId) {
        if (this.auth.user?.role === 'HOD') {
          this.performanceService.getHoScores(this.period, this.HODId, this.branchId).subscribe((data:any) => {
            this.hodScores = data.scores;
          });
        } else if (this.auth.user?.role === 'HO_STAFF' || this.auth.user?.role === 'attender') {
          const userId = this.auth.user?.id ?? '';
          this.performanceService.getStaffScores(this.period, this.auth.user?.id,this.branchId).subscribe(data => {
  ...
  loadScores() {
     this.performanceService.getScores(this.period, this.HODId, this.branchId).subscribe((data: any) => {
  this.scores = data;
